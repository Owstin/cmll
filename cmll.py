...
def addAlg(fileText, algName, algText):
    LF = "\n"

    colons = fileText.count(":")
    pos = 0
    cPos = 0
    for i in range(0, TOTAL_CASES):
        cPos = fileText.find(":", pos)
        if(fileText[cPos-2:cPos] == algName):
            break
        pos = cPos+1

    newLine = fileText.find("\n\n", cPos)
    return fileText[:newLine+1] + algText + LF + fileText[newLine+1:]

def generateJsText(fileText):
...
#jsFile.write(jsStr)
#jsFile.close()
jsPath = os.path.join(cwd, scriptsDir, changeExt(cmllFileName, "js"))

cmllText = readInputFile(cmllFileName)
cmllText = addAlg(cmllText, "c1", "r U R' U R U2 r'")

jsText = generateJsText(cmllText)

updateFile(cmllFileName, cmllText)
updateFile(jsPath, jsText)
